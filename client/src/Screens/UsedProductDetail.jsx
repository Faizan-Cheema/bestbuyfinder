...
const UsedProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:5000/api/searchNewProductById?pid=${id}`);
        if (!response.ok) {
    ...
    <div className="bg-gray-100 min-h-screen p-8">
      ...
      <div className="flex flex-col md:flex-row px-4 md:px-20 py-8">
        <img
          ...
          src={`http://localhost:5000/uploads/${product.productPhotos[0]}`}
        ...
        <div className="bg-gray-50 md:pl-12 flex-1 p-8 md:p-0">
          ...
          <div className="flex items-center pt-4">
            ...
            <p className="pl-2 pt-1">{product.location}</p>
          </div>

          <div className="flex items-center pt-2">
            <h3 className="text-lg font-semibold">Phone Number:</h3>
            <p className="pl-2 pt-1">{product.phoneNumber}</p>
          </div>
