    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/searchNewProducts?search=${query}&skip=${skip}&limit=${perPage}`
        );
      ...
      <div className="product-list">
        {products.length > 0 ? (
          products.slice(startIndex, endIndex).map((product) => (
            <Link key={product._id} to={`/usedproductdetails?pid=${product._id}`}>
              <ProductCard
                name={product.name}
                price={`Rs. ${product.price}`}
                website={product.source}
                imgSrc={`http://localhost:5000/uploads/${product.productPhotos[0]}`}
