...
function Products() {
  ...
  return (
    <div>
      ...
      <div className="product-list grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
        {products.length > 0 ? (
          products.slice(startIndex, endIndex).map((product) => {
            if (product.price <= 0) {
              return null;
            }
            const imageSrc = product.image.startsWith('https://www.shophive.') ? defaultimage : product.image;
            return (
